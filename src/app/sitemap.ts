...
async function collectSlugs(locale: string) {
  const [productsRes, projectsRes, servicesRes, categoriesRes] =
    await Promise.all([
  ...
  return {
    ...
    categorySlugs: categories
      .map((item) => pickSlug(item.slug, locale))
      .filter(Boolean),
  };
}
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  for (const locale of routing.locales) {
    const { productSlugs, projectSlugs, serviceSlugs, categorySlugs } =
      await collectSlugs(locale);
